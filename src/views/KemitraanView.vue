            ...
            <table class="min-w-full leading-normal">
              <thead>
                ...
                <tr>
                  ...
                  <th class="px-5 py-3 text-xs font-semibold tracking-wider text-left text-gray-600 uppercase bg-gray-100 border-b-2 border-gray-200">MoU</th>
              ...
              <tbody>
                ...
                <tr v-else v-for="item in computedData" :key="item.id">
                  <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                    <div class="flex items-center">
                      <div class="flex-shrink-0 w-10 h-10">
                        <img v-if="item?.image" class="w-[50px] h-[50px] rounded-[4px]" :src="item.image" alt="kemitraan image" />
                  ...
                  <td class="px-5 py-5 text-sm bg-white border-b border-gray-200">
                    <a v-if="item?.mou" :href="item.mou" target="_blank" class="text-blue-600 underline text-sm">Lihat MoU</a>
...
import { GET_KEMITRAANS, DELETE_KEMITRAAN } from "@/store/kemitraan.module";
...
const pagination = computed(() => {
  const kemitraans = store.getters.kemitraans;
  return kemitraans?.pagination || [];
});

const getData = async () => {
  const params = { search: search.value, limit: limit.value, page: page.value };
  try {
    const data = await store.dispatch(GET_KEMITRAANS, params);
    return data;
  } catch (err) {
    console.error(err);
  } finally {
